...
VALUES ('{{ ti.xcom_pull(task_ids=params.snapshot_task_id) }}'::uuid, now(), '{{ params.pipeline_id }}', 'success', 'snapshot')
...
SELECT
  '{{ ti.xcom_pull(task_ids=params.snapshot_task_id) }}'::uuid, now(), '{{ params.pipeline_id }}',
